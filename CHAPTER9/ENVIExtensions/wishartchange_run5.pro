...
pro wishartchange_run5_extensions_init
   compile_opt idl2
   e = envi(/current)
   e.addextension, 'Complex Wishart Change Detection', 'wishartchange_run5', path = 'Chapter9'
end
...
; calculate determinants and change statistic distribution parameters  
   case bands1 of
     ...
     4: begin
       print, 'c11-> '+bnames1[0]+' c12re-> '+bnames1[1]+' c12im-> '+bnames1[2]+' c22-> '+bnames1[3]  
       k1 = n1*envi_get_data(fid=fid1,dims=dims1,pos=0) ;c11
       a1 = envi_get_data(fid=fid1,dims=dims1,pos=1) ;c12
       im = envi_get_data(fid=fid1,dims=dims1,pos=2)
       a1 = n1*complex(a1,im)
...
; write to memory or disk  
   if (result.outf.in_memory eq 1) then begin
      ...
      envi_setup_head,fname=result.outf.name, ns=samples, nl=lines, nb=4, $
                    data_type=4, $
                    interleave=0, $
                    file_type=0, $  
                    bnames = bnames, $  
