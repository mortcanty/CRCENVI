...
pro atwt_run5_extensions_init
   compile_opt idl2
   e = envi(/current)
   e.addextension, 'ATWT Pansharpening', 'atwt_run5', path='Chapter5'
end
...
print, '---------------------------------'
print, 'ATWT Panchromatic sharpening'
print, systime(0)
print, '---------------------------------'
...
; resize the ms band to scale of the pan image using NN resampling
      ...
      tmp = envifidtoraster(r_fid)
      tmp.close
      tmp = envifidtoraster(r_fid1)
      tmp.close
