...
PRO ffn_run_define_buttons, buttonInfo
   ENVI_DEFINE_MENU_BUTTON, buttonInfo, $
      VALUE = 'Neural Net (Kalman/Conj Gradient)', $
...
;       neural network using Kalman filter
...
; validation flag
 query=dialog_message('Use validation',/question)
 if query eq 'Yes' then valid_flag=1 else valid_flag=0
...
; train the network
   ...
   print, 'Training with Kalman filter...'
   FFN = Obj_New('FFNKAL',transpose(Gs),transpose(Ls),n_hu)
   FFN->train, key=valid_flag
   wts = FFN->GetWeights()
   Obj_Destroy, FFN
   print, 'Training with scaled conjugate gradient...'
   FFN = Obj_New('FFNCG',transpose(Gs),transpose(Ls),n_hu)
   FFN->PutWeights,wts
   FFN->train, key=valid_flag
   print, '... done'

   query=dialog_message('Calculate Hessian Eigenvalues',/question)
   if query eq 'Yes' then $
      envi_info_wid,string( FFN->Eigenvalues(), format='(4F10.2)'),  $
