...
; GPU arrays
;  work
gpuPutArr,fltarr(n)+1,onesn_gpu
gpuPutArr,fltArr(NN)+1,onesNN_gpu
;  data matrix
gpuPutArr,float(G),G_gpu
;  membership matrix
gpuPutArr,float(U),U_gpu
gpuputArr,float(unfrozen),unfrozen_gpu
...
;    jth covariance matrix      
      ...
      for i=0,NN-1 do begin
         ...
         gpuMult,tmp_gpu,Dsti_gpu,Wti_gpu               
      endfor   
      gpuFree,tmp_gpu 
      gpuMatrix_Multiply,Wt_gpu,Wt_gpu,C_gpu,/atranspose 
      gpuFree,Wt_gpu
      gpuAdd,1.0/ns[j],C_gpu,0.0,C_gpu,0.0,C_gpu 
...
;    new memberships               
      qf_gpu = gpuTotal(Dst_gpu*(Cinv_gpu##Dst_gpu),2) 
      gpuFree,[Cinv_gpu,Dst_gpu]
      qfu_gpu = gpuSubscript(unfrozen_gpu,qf_gpu)
      tmp_gpu = gpuExp(Ps[j]/sqrtdetC,-0.5,qfu_gpu,0.0,0.0)
      gpuFree,qfu_gpu
...
;    random membership for annealing
      if T gt 0.0 then begin
         ...
         gpuFree, [Ur_gpu, pow_gpu, Ursub_gpu] 
      endif
   endfor 
...
;         V[*,j] = exp(-beta*U_N)  
         ...
         gpuAdd, -beta, Vj_gpu, 0.0, Vj_gpu, 0.0, Vj_gpu
         
         Vj_gpu = gpuExp(Vj_gpu) 
...
; stopping criterion  
   ...
   if (n_elements(wnd) eq 1) and (iter mod 5 eq 0) then $
     plot, LL[0:iter-1], color=0, background= 'FFFFFF'XL, xtitle='EM Iteration',position=position
